<?php
  if(!isset($_SESSION)) { 
	...
	if (!$row = $result->fetch_assoc()) {
		...
		if($row['rank'] == 'Admin' || $row['rank'] == 'prof' || $row['rank'] == 'Supervisor' || $row['rank'] == 'supervisor' || $row['rank'] == 'Student' || $row['rank'] == 'student') {
			session_start();
			$_SESSION['rank'] = $row['rank'];

			if(isset($_SESSION['rank'])) {
				if($_SESSION['rank'] == 'Admin' || $row['rank'] == 'prof') {
					header("Location: home/profHome.php?id=$username");
				}
