            ...
            <table class="table">
                ...
                <tbody>
                    <?php
                        ...
                        {
                            $lecturerName = $_SESSION['id'];
                            $lecturerIdQuery= "
                              SELECT id FROM users where name='$lecturerName'
                             ";
                            $lecturerRecord = mysqli_query($conn, $lecturerIdQuery);
                            $row = mysqli_fetch_array($lecturerRecord);
                            $query = "
                            SELECT *
                            FROM marking_scheme_marks m
                            LEFT JOIN module d
                            ON m.module_code = d.module_code
                            LEFT JOIN assessment a
                            ON m.assessment_code = a.assessment_code
                            WHERE lecturer_id = ".$row['id'].";
                         ";
                        }
...
//                             exit;
                             ...
                             $studentName = "
                          SELECT name, surname FROM users where id='$studentId'
                         ";
                             $result2 = mysqli_query($conn, $studentName);
                             $row2 = mysqli_fetch_array($result2);
                             $output .= '
                           <tr>
                            <td>'.$row3[2].'</td>
                            <td>'.$row3["module_name"].'</td>
                            <td>'.$row2["name"].' '.$row2['surname'].'</td>
                            <td>'.$row3[3].'</td>
                            <td>
		                        <button type="button" class="btn btn-success"><a href="../lecturer/add-single-mark.php?id=' . $row3["lecturer_id"] . '">Add Mark</a></button>
		                    </td>
                           </tr>
                          ';
                         }
